...
namespace MGTK.Controls
{
    public class InnerTextBox : Control
    {
        ...
        private int MSToNeMGTKeyPress = 500;
        ...
        void InnerTextBox_MouseLeftPressed(object sender, EventArgs e)
        {
            ...
            {
                if (Font.MeasureString(internalText[CursorY].Text.Substring(pSX, i)) < (WindowManager.MouseX - (OwnerX + X)))
                    CursorX = pSX + i;
                else
                    break;
            }
        ...
        private bool CanRegisterKey(Keys keypressed)
        {
            ...
            if (tbkp != null)
            {
                if (gameTime.TotalGameTime.TotalMilliseconds - tbkp.LastUpdate <= MSToNeMGTKeyPress / (tbkp.nrKeyPressesRegistered > 1 ? 16 : 1))
                    return false;
            }
        ...
        private void KeyChangeCursorY(bool up)
        {
            ...
            currentWidthToCursorX = Font.MeasureString(internalText[CursorY].Text.Substring(ScrollX, CursorX - ScrollX));

            CursorY += up ? -1 : 1;

            for (i = 0; i < internalText[CursorY].Text.Length - pSX; i++)
            {
                if (Font.MeasureString(internalText[CursorY].Text.Substring(pSX, i)) > currentWidthToCursorX)
                    CursorX = pSX + i;
                else
                    break;
            }
        ...
        private void UpdateScrollHorizontally()
        {
            ...
            if (ScrollX < CursorX)
            {
                stringSize = Font.MeasureString(internalText[CursorY].Text.Substring(ScrollX, CursorX - ScrollX)) + 1;
        ...
        private void UpdateNrCharsVisible()
        {
            ...
            for (int i = 0; i < internalText.Count; i++)
            {
                ...
                while (Font.MeasureString(itemText.Substring(0, internalText[i].NrCharsVisible)) + TextXPadding > Width &&
                    internalText[i].NrCharsVisible > 0)
                {
